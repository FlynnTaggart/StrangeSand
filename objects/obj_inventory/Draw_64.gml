...
var info_grid = global.ds_player_info;
...
var ix = 0, iy = 0, xx, yy, i_item, inv_grid = global.ds_inventory, sx, sy;

for(var i = 0; i < global.inv_slots; ++i){
	ix = i mod global.inv_slots_width;
	iy = i div global.inv_slots_width;
...
if(!mouse_in_weapon_slots){
	...
	if(i_item > -1){
		...
		draw_text_ext_transformed_color(desc_x, desc_y, description, string_height("M"), 130 * scale * 2, t_scale, t_scale, 0, c, c, c, c, 1);
	}
}
else{
	for(var i = 0; i < 4; ++i){
		...
		sx = (i_item mod spr_inv_items_columns) * cell_size;
		sy = (i_item div spr_inv_items_columns) * cell_size;
		if(i == selected_weapon_slot && i_item > 0)
			draw_sprite_part_ext(spr_inv_items, 0, sx, sy, cell_size, cell_size, inv_UI_x + 145 + 34 * i, inv_UI_y + 158, scale, scale, c_white, 0.4);
		else if(i_item > 0)
			draw_sprite_part_ext(spr_inv_items, 0, sx, sy, cell_size, cell_size, inv_UI_x + 145 + 34 * i, inv_UI_y + 158, scale, scale, c_white, 1);
		if(i == selected_weapon_slot){
			gpu_set_blendmode(bm_add);
			draw_sprite_part_ext(spr_inv_UI, 0, 0, 0, cell_size, cell_size, inv_UI_x + 145 + 34 * selected_weapon_slot, inv_UI_y + 158, scale, scale, c_white, 0.4);
			gpu_set_blendmode(bm_normal);
		}
	}
}
for(var i = 0; i < 4; ++i){
	...
	sx = (i_item mod spr_inv_items_columns) * cell_size;
	sy = (i_item div spr_inv_items_columns) * cell_size;
	if(i == weapon_pickup_slot && i_item > 0)
		draw_sprite_part_ext(spr_inv_items, 0, sx, sy, cell_size, cell_size, inv_UI_x + 145 + 34 * i, inv_UI_y + 158, scale, scale, c_white, 0.4);
	else if(i_item > 0)
		draw_sprite_part_ext(spr_inv_items, 0, sx, sy, cell_size, cell_size, inv_UI_x + 145 + 34 * i, inv_UI_y + 158, scale, scale, c_white, 1);
}
if(pickup_slot != -1){
	...
	draw_set_font(fnt_smalldigits);
	if(inv_grid[# 0, pickup_slot] != item.arifle && inv_grid[# 0, pickup_slot] != item.revolver && inv_grid[# 0, pickup_slot] != item.pipe){
